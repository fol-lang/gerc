...
pub fn emit_build_rs(proj: &RustProjection) -> String {
    use crate::ir::RustLinkKind;

    let mut out = String::new();
    out.push_str("// Generated by gec — do not edit.\nfn main() {\n");

    for req in &proj.link_requirements {
...
mod tests {
    ...
    fn emit_crate_creates_lib_rs() {
        ...
        let cfg = sample_config();
        let result = emit_crate(&proj, &cfg, &dir, OutputMode::Crate, OverwritePolicy::Overwrite).unwrap();
    ...
    fn emit_crate_creates_cargo_toml() {
        ...
        let proj = sample_projection();
        let cfg = sample_config();
        emit_crate(&proj, &cfg, &dir, OutputMode::Crate, OverwritePolicy::Overwrite).unwrap();
    ...
    fn emit_source_bundle_no_cargo_toml() {
        ...
        let proj = sample_projection();
        let cfg = sample_config();
        emit_crate(&proj, &cfg, &dir, OutputMode::SourceBundle, OverwritePolicy::Overwrite).unwrap();
